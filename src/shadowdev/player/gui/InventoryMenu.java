...
public class InventoryMenu extends PlayerGui implements InventoryListener {

	List<ItemCustom> icons = new ArrayList<ItemCustom>();
	List<Integer> amts = new ArrayList<Integer>();
	int start = 0;
	
	public InventoryMenu(Player p, int start) {
		super (p, ChatColor.GREEN + "Inventory", 54);
		...
		for (int i = 0; i < ServerManager.getPlayer(p).getInventory().size(); i++) {
			...
			if (item instanceof ToolX || item instanceof ArmorX || item instanceof PotionX) continue;
			int amt = 0;
	...
	public void onClick(Player p, ItemStack i, int slot, ClickType click, Inventory inv) {
		...
		if (i != null && slot == 47) {
			start += 12;
			InventoryMenu im = new InventoryMenu(p, start);
			im.send();
		}
		if (i != null && slot == 2) {
			start -= 12;
		...
		if (c != null && c instanceof FoodX) {
			...
			InventoryMenu m = new InventoryMenu(p, start);
			m.send();
		}
	}

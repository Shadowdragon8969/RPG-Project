...
public class PotionMenu extends PlayerGui implements InventoryListener {

	List<ItemCustom> icons = new ArrayList<ItemCustom>();
	List<Integer> amts = new ArrayList<Integer>();
	int start = 0;
	
	public PotionMenu(Player p, int start) {
		super (p, ChatColor.GREEN + "Potions and Crystals", 54);
		...
		//setItem(47, Material.GOLD_NUGGET, 1, ChatColor.GOLD + "コル" + ": " + gp.getAttribute("cor").getLevel(), "This is your bank balance");
		
		this.start = start;
		for (int i = 0; i < ServerManager.getPlayer(p).getInventory().size(); i++) {
			ItemCustom item = ServerManager.getPlayer(p).getInventory().get(i);
			if (!(item instanceof PotionX)) continue;
			int amt = 0;
		...
		setItem(41, Material.GREEN_STAINED_GLASS_PANE, 1, " ", "");
		setItem(43, Material.GREEN_STAINED_GLASS_PANE, 1, " ", "");
		if (start > 0) {
	...
	public void onClick(Player p, ItemStack i, int slot, ClickType click, Inventory inv) {
		...
		if (i != null && slot == 47) {
			start += 12;
			PotionMenu im = new PotionMenu(p, start);
			im.send();
		}
		if (i != null && slot == 2) {
			start -= 12;
		...
		if (c != null && c instanceof PotionX) {
			PotionX f = (PotionX) c;
			f.onInteract(p);
			ServerManager.getPlayer(p).removeItem(c);
			PotionMenu m = new PotionMenu(p, start);
			m.send();
		}
	}
